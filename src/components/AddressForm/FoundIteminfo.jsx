...
const FoundIteminfo = ({
  ...
  const data = getAddressData?.find(
    (item) => item.Type === savedAddressInfo.text
  );
        ...
        <Card
          className={
            savedAddressIndex === addressButtonIndex
              ? "mb-16 hover-card pointer hover-Effect"
              : "mb-16 hover-card pointer"
          }
          onClick={selectShippingAddressHandler.bind(
            this,
            savedAddressIndex,
            addressButtonIndex,
            data,
        ...
        >
          {pathname === "/profile/address" ? (
            <div className="span hover-card pointer">
              <h4 className="t-16 t-bold mb-8">
                {savedAddressInfo.text} Address
              </h4>
              <p className=" mb-4">
                {data.Name}-{data.Mobile}-{data.Email}
              </p>
              <p>
                {data.Province},{data.District},{data.Upazila}
              </p>
              <p>{data.Remarks}</p>
            </div>
          ) : (
            <div
              className="span hover-card pointer"
